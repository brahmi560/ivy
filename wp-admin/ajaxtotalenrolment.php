...
?>
			...
			global $wpdb;
            $results = $wpdb->get_results( 'SELECT a.*,b.*,c.post_title FROM wp_488a9xj6dq_gdlrpayment a INNER JOIN wp_488a9xj6dq_users b ON(a.student_Id = b.ID) INNER JOIN wp_488a9xj6dq_posts c ON(a.course_id = c.ID) WHERE DATE(a.payment_date) BETWEEN "'.$mindate.'" AND "'.$maxdate.'"  ORDER BY a.payment_date DESC', OBJECT );
			//print_r($results);
			?>
