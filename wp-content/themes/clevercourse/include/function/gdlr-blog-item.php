	...
	*/
	...
	if( !function_exists('gdlr_carousel_grid_nav') ){
		function gdlr_carousel_grid_nav(){
			...
			$ret .= '<span class="gdlr-flex-prev" ><i class="fa fa-angle-left icon-angle-left"></i></span>';
			$ret .= '<span class="gdlr-flex-next" ><i class="fa fa-angle-right icon-angle-right"></i></span>';
			$ret .= '</div>';
	...
	if( !function_exists('gdlr_get_blog_info') ){
		function gdlr_get_blog_info( $array = array(), $wrapper = true, $sep = '' ){
			...
			foreach($array as $post_info){
				...
				switch( $post_info ){
					case 'date':
						$ret2 .= '<div class="blog-info blog-date">';
						$ret2 .= '<span class="gdlr-head">' . __('Posted on', 'gdlr_translate') . '</span>';
						$ret2 .= '<a href="' . get_day_link( get_the_time('Y'), get_the_time('m'), get_the_time('d')) . '">';
						$ret2 .= get_the_time($theme_option['date-format']);
						$ret2 .= '</a>';
						$ret2 .= '</div>';
						break;
					case 'tag':
						...
						$ret2 .= '<div class="blog-info blog-tag">';
						$ret2 .= '<span class="gdlr-head">' . __('Tags', 'gdlr_translate') . '</span>';
						$ret2 .= $tag;						
						$ret2 .= '</div>';						
						break;
					case 'category':
						...
						$ret2 .= '<div class="blog-info blog-category">';
						$ret2 .= '<span class="gdlr-head">' . __('Categories', 'gdlr_translate') . '</span>';
						$ret2 .= $category;					
						$ret2 .= '</div>';					
						break;
					case 'comment':
						$ret2 .= '<div class="blog-info blog-comment">';
						$ret2 .= '<span class="gdlr-head">' . __('Comments', 'gdlr_translate') . '</span>';
						$ret2 .= '<a href="' . get_permalink() . '#respond" >' . get_comments_number() . '</a>';						
						$ret2 .= '</div>';						
						break;
					case 'author':
						ob_start();
						...
						$ret2 .= '<div class="blog-info blog-author">';
						$ret2 .= '<span class="gdlr-head">' . __('Posted By', 'gdlr_translate') . '</span>';
						$ret2 .= $author;
						$ret2 .= '</div>';						
						break;						
				}
				if( !empty($ret2) ) $ret .= $sep . $ret2;
			}
