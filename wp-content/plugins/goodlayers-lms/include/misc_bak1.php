	...
	*/
	...
	function gdlr_lms_header_signin(){
		if( is_user_logged_in() ){
			...
			echo '<a data-rel="gdlr-lms-lightbox" data-lb-open="login-form" href="http://localhost/IVYGA_ADMIN/wp-login.php" >' . __('Sign In', 'gdlr-lms') . '</a>';
			//gdlr_lms_sign_in_lightbox_form();
			echo '<a href="' . esc_url(add_query_arg('register', get_permalink(), home_url())) . '">' . __('', 'gdlr-lms') . '</a>';
			echo '</div>';		
		}
	}
